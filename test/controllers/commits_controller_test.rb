...
class CommitsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create commit" do
    assert_difference('Commit.count') do
      post commits_url, params: { commit: { content: @commit.content, micropost_id: @commit.micropost_id } }
    end
  ...
  test "should update commit" do
    patch commit_url(@commit), params: { commit: { content: @commit.content, micropost_id: @commit.micropost_id } }
